...
router.post("/performer", async (req, res) => {
  try {
    ...
    if (!roomModel)
      return res.status(400).send({ error: "Room not found in database" });

    roomModel.gameOn = true;
    await roomModel.save();

    let connectedParticipants = await client.video
      .rooms(req.body.room)
      .participants.list({ status: "connected" });
    connectedParticipants = connectedParticipants.map(
    ...
    if (connectedParticipants.length === 1)
      return res
        .status(400)
        .send({ error: "Find some friends to play with :)" });
...
router.post("/task_giver", async (req, res) => {
  ...
  if (connectedParticipants.length === 0)
    return res.status(400).send({ error: "No other participant found" });
...
router.post("/score_update", async (req, res) => {
  ...
  if (!participant)
    return res.status(400).send({ error: "Unable to update score" });

  // get other participants
  const otherParticipants = roomModel.participants.filter(
    (participant) => participant.name !== req.body.identity
  );

  // console.log(`2. ${otherParticipants}`);

  // update performer score
  if (req.body.score === 1) participant.score = participant.score + 1;
...
router.post("/get_winners", async (req, res) => {
  const { error } = validateGetWinner(req.body);
  if (error) return res.status(400).send({ error: error.details[0].message });
  ...
  try {
    ...
    // only single participant connected
    if (connectedParticipants.length === 1)
      return res
        .status(400)
        .send({ error: "All other players left the room :(" });

    // double check
    const roomModalConnectedParticipants = roomModel.participants.filter(
      (participant) => {
        return connectedParticipants.includes(participant.name);
      }
    );

    // get the maximum score
    const scoreList = roomModalConnectedParticipants.map(
      (participant) => participant.score
    );
    const maxScore = Math.max(...scoreList);

    // get the name of all the winner(s)
    const winners = roomModalConnectedParticipants
      .filter((participant) => {
        return participant.score === maxScore;
      })
      .map((participant) => participant.name);

    // zeroing all members score whether connected or not
    for (let i = 0; i < roomModel.participants.length; i++) {
      roomModel.participants[i].score = 0;
    }
